    ...
    this.$get = ['$rootScope', '$window', '$document', '$parse','$timeout', function($rootScope, $window, $document, $parse, $timeout) {
      ...
      var addToLocalStorage = function (key, value,type) {
        ...
        if (isUndefined(value)) {
          ...
          //console.log('before json'+value);
          value = toJson(value);
          //console.log('after json'+value);
        }
      ...
      var removeFromLocalStorageKey = function (value) {
        // can't pop on arguments, so we do this
        var consumed = 0;
        if (arguments.length >= 1 &&
            (arguments[arguments.length - 1] === 'localStorage' ||
      ...
      var clearAKeyFromLocalStorage = function (regularExpression, type,toDeleteKey) {
        setStorageType(type);
        ...
        for (var key in webStorage) {
          ...
          if (prefixRegex.test(key) && testRegex.test(key.substr(prefixLength))) {
            try {
              removeFromLocalStorageKey(key.substr(prefixLength),toDeleteKey);
            } catch (e) {
              $rootScope.$broadcast('LocalStorageModule.notification.error', e.message);
              //return clearAllFromCookies();
            }
      ...
      var browserSupportsCookies = (function() {
        ...
        return {
          ...
          removeKey: removeFromLocalStorageKey,
          clearAll: clearAllFromLocalStorage,
          clearAKey: clearAKeyFromLocalStorage,
          bind: bindToScope,
